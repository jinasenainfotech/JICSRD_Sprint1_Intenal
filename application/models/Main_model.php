 ...
 */
	...
	public function report_row($com){
		...
		$query = $this->db->query("SELECT `quantitave_input`.*, `company_information`.`entity_name`	FROM`company_information` INNER JOIN `quantitave_input` ON (`company_information`.`id` = `quantitave_input`.`company_name`) WHERE company_name = '". $com ."' ORDER BY `quantitave_input`.`financial_year` ASC LIMIT 3;");
