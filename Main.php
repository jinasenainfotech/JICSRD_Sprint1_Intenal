...
*/
	...
	public function newreport(){
		...
		// $this->form_validation->set_rules('report_type', 'report_type', 'required');
		...
		if ($this->form_validation->run() == TRUE) {
			...
			for ($r = 0; $r < $i; $r++) { 
				...
				if($this->input->post('report_type')==null){
					$this->jics->alert('error','Report Type is Required Field','Save');
				...
				$data = array(
					...
					'financial_perfomance'				=> (isset($_POST['financial_perfomance']))? $_POST['financial_perfomance'] :'',
				);
...
$x127 = ((floatval($_POST['total_assets'][$r])!=0) || (floatval($_POST['total_liabilities'][$r])!=0))?((($x74/floatval($_POST['total_assets'][$r]))*1.2)+((floatval($_POST['ratained_earning'][$r])/floatval($_POST['total_assets'][$r]))*1.4)+((floatval($_POST['ebit'][$r])/floatval($_POST['total_assets'][$r]))*3.3)+((floatval($_POST['total_equity'][$r])/floatval($_POST['total_liabilities'][$r]))*0.6)+((floatval($_POST['sales'][$r])/floatval($_POST['total_assets'][$r])))) : 0;
...
$cal = array(
	...
	'cash_flow_coverage'				=> (floatval($_POST['total_current_liabilities'][$r])!=0)?((floatval($_POST['operating_cash_flow'][$r])/floatval($_POST['total_current_liabilities'][$r]))*100):0,
	...
	'net_asset_backing'					=> ($x112!=0)?((isset($x112) and $x112 != '')? ((floatval($_POST['total_assets'][$r]) - floatval($_POST['total_liabilities'][$r]) - floatval($_POST['net_intangibles'][$r])) / $x112) * 100 : ""):0 ,
	...
	'debt_to_equity'					=> (floatval($_POST['total_equity'][$r])!=0)?((floatval($_POST['interest_bearing_debt_1'][$r]) +floatval($_POST['lone_from_related_parties'][$r] )+ floatval($_POST['total_current_liabilities_2'][$r]) +floatval($_POST['loans_from_related_parites'][$r]))/ floatval($_POST['total_equity'][$r])):0,
	...
	'short_ratio'						=>((floatval($_POST['interest_bearing_debt_1'][$r]) +floatval($_POST['lone_from_related_parties'][$r] )+ floatval($_POST['total_current_liabilities_2'][$r]) +floatval($_POST['loans_from_related_parites'][$r]))!=0)?((floatval($_POST['interest_bearing_debt_1'][$r]) +floatval($_POST['lone_from_related_parties'][$r] ))/(floatval($_POST['interest_bearing_debt_1'][$r]) +floatval($_POST['lone_from_related_parties'][$r] )+ floatval($_POST['total_current_liabilities_2'][$r]) +floatval($_POST['loans_from_related_parites'][$r]))*100):0,
	...
	'current_asset_composition'			=> (floatval($_POST['total_assets'][$r])!=0)?((floatval($_POST['total_current_assets'][$r]) / floatval($_POST['total_assets'][$r])) * 100):0,
	'current_liability_composition'		=> (floatval($_POST['total_liabilities'][$r])!=0)?(((floatval($_POST['total_current_liabilities'][$r]) / floatval($_POST['total_liabilities'][$r]))) * 100):0,
...
public function username($username = '')
    {
        ...
        if (empty($username))
        {
            $this->form_validation->set_message('username', 'The {field} field is required.');
            return FALSE;
        }
        if ((preg_match_all($regex_lowercase, $username) < 1))
        {
            $this->form_validation->set_message('username', 'The {field} field must be at least one letter.');
            return FALSE;
		}
		if ((preg_match_all($spaces, $username) > 0))
        {
            $this->form_validation->set_message('username', 'The {field} field must be without any spaces.');
            return FALSE;
        }
...
public function users(){
	...
	$this->form_validation->set_rules('user_name', 'Use Name', 'required|trim|min_length[4]|max_length[10]|callback_username');
	$this->form_validation->set_rules('first_name', 'First Name', 'required');
	$this->form_validation->set_rules('password', 'Password', 'trim|min_length[6]|max_length[16]|callback_password');
